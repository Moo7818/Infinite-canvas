...
+ [修复] 画布生成前参考图下载失败不再是控制台未捕获错误，会转为中文提示并中断生成；重试路径同样处理。
+ [调整] Grsai 图片/视频调用脚本兼容多种任务状态与返回包壳，轮询遇到无法识别的返回结构直接报错（附原始内容预览），不再空轮询至超时。
+ [修复] 生成的音视频远端文件下载增加 10 分钟超时并支持取消，超时自动降级为远端地址，不再无限卡在生成中。
+ [修复] Grsai 调用脚本轮询时上游以 HTTP 400 返回失败，错误体经解析后展示为中文任务失败原因；轮询等待增加控制台 debug 心跳，便于区分缓慢与卡死。
+ [新增] 自定义脚本支持 onProgress(ratio) 上报轮询进度，画布 loading 节点会显示上游真实进度百分比；Grsai 图片/视频脚本已接入。
+ [新增] 渠道模型支持自定义参数控件（customParams：下拉/数字/文本），画布节点参数选择器按模型声明动态渲染，取值以 `params.<key>` 透传脚本。

## v0.18.1 - 2026-09-19

+ [新增] 自有角色节点插件（`character:role`）：表单填写角色要素并拼装提示词，用参考图生成半身近景+全身三视图设定图。
+ [新增] 自有场景节点插件（`scene:view`）：表单填写场景要素并拼装提示词，用参考图生成电影质感场景图。
+ [新增] 自有道具节点插件（`prop:item`）：表单填写道具要素并拼装提示词，用参考图生成产品级道具图。
+ [新增] 自有宫格调度节点插件（`grid:board`）：锁死 2*2 四宫格，按名引用角色/空间节点，调度文档拼装提示词一次生成四帧静帧。
+ [新增] Grsai 渠道模型配置资产（`docs/assets/grsai-autodl-canvas-config.json`）：16 个生图模型脚本统一 `replyType: async` 快返任务 id 再轮询 `/v1/api/result`，慢模型不再被网关掐断连接；含 minimax-h3 视频脚本与双文本脚本，可整包导入。
+ [新增] 插件宿主接口扩展：`updateMetadata` 函数式重载（并发读改写安全）、`panelPlacement` 面板方位、`hasTargetHandle` 输入连接点开关、右键菜单 `pluginItems` 插槽，插件运行时改为先初始化后注册。
