...
export type ContextMenuPluginItem = { id: string; label: string; icon?: ReactNode; onClick: () => void };

export function CanvasNodeContextMenu({ menu, canCaptureVideoFrame, canGroup, canUngroup, pluginItems, onClose, onCaptureVideoFrame, onDuplicate, onGroup, onUngroup, onDelete }: { menu: ContextMenuState; canCaptureVideoFrame: boolean; canGroup?: boolean; canUngroup?: boolean; pluginItems?: ContextMenuPluginItem[]; onClose: () => void; onCaptureVideoFrame: (position: VideoFramePosition) => void; onDuplicate: () => void; onGroup?: () => void; onUngroup?: () => void; onDelete: () => void }) {
    ...
    return (
        ...
        >
            ...
            {menu.type === "node" && pluginItems?.length ? (
                <>
                    <div className="my-1 border-t" style={{ borderColor: theme.toolbar.border }} />
                    {pluginItems.map((item) => (
                        <MenuButton key={item.id} icon={item.icon} label={item.label} onClick={item.onClick} />
                    ))}
                </>
            ) : null}
